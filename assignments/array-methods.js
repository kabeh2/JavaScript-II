...
// Get an array of all the donations and push to ticketPriceTotal
runners.map(el => ticketPriceTotal.push(el.donation));

// Sum up total of array using reduce method
let sum = ticketPriceTotal.reduce((a, b) => a + b);

///////////////////////////////////////////
/////////// USING ONLY MAP (BUT I KNOW REDUCE IS A BETTER USE CASE)
// runners.map(el => {
//   ticketPriceTotal += el.donation;
// });
...
// Create an array of all the emails so we can send out group emails
let emailRunners = [];

runners.map(el => {
  emailRunners.push(el.email);
});
...
// Filter out the big donors that donated $100 or more and log the list of their profiles.
let bigDonors = runners.map(el => {
  if (el.donation >= 100) {
    return `${el.first_name} ${el.last_name} donated $${el.donation}`;
  }
});
...
// Log the profiles of all those that donated under $100

let smallDonors = runners.filter(el => {
  return el.donation < 100;
});
